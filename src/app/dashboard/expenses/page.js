...
export default function ExpensesPage() {
  ...
  // Initialize walletId when wallets load
  useEffect(() => {
    if (wallets && wallets.data && wallets.data.length > 0) {
      setFormData(prev => ({ ...prev, walletId: null }));
    }
  }, [wallets]);
  ...
  const openAddModal = () => {
    ...
    setFormData({
      description: "",
      amount: "",
      walletId: wallets?.data?.length > 0 ? null : undefined,
      currency: DEFAULT_CURRENCY,
      date: new Date(),
    });
  ...
  const openEditModal = (expense) => {
    ...
    setFormData({
      ...
      walletId: expense.fk_wallet_id || expense.walletId || null,
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select 
                        ...
                        onChange={(e) => setFormData({ ...formData, walletId: e.target.value || null })}
                        disabled={isSubmitting || !wallets?.data?.length}
                      >
